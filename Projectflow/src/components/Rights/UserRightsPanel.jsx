...
import React, { useState, useEffect } from "react"; import { ROLES, SCREENS, ACTIONS, DEFAULT_PERMISSIONS } from "../../constants/roles"; import { Btn, Badge, Input, Field } from "../UI/UI";

// ── Emojis disponibles pour les avatars const AVATAR_LIST = [
  "🦊","🐻","🦋","🐬","🦁","🐯","🦊","🐺","🦝","🐸",
  "🦄","🐧","🦅","🦜","🐙","🦈","🌟","🚀","⚡","🎯",
  "🔥","💎","🌈","🎨","🎭","🏆","💡","🎸","🌙","☀️",
];
...
// ── Stockage local des utilisateurs dynamiques const USERS_KEY = "projectflow-users-v1";
...
export function UserRightsPanel({ profiles, onSave, currentUser }) {
  ...
  const handleCreateUser = () => {
    ...
    if (users.find(u => u.email.toLowerCase() === userForm.email.toLowerCase())) return setFormError("Cet email existe déjà.");

    const newId   = Math.max(...users.map(u => u.id), 0) + 1;
    const newUser = { id: newId, name: userForm.name.trim(), email: userForm.email.trim(), password: userForm.password.trim(), avatar: userForm.avatar, color: userForm.color };
      ...
      <div style={{ width: 240, borderRight: "1px solid #F1F3F5", flexShrink: 0, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "16px 16px 10px", borderBottom: "1px solid #F1F3F5" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 2 }}>
            ...
            {isAdmin && (
              <button onClick={() => { setShowCreate(true); setUserForm(emptyForm); setFormError(""); }}
          ...
          {users.map(acc => {
            ...
            return (
              <div key={acc.id} onClick={() => setSelectedUser(acc.id)}
                ...
                <div style={{ width: 34, height: 34, borderRadius: "50%", background: acc.color + "22", border: `1.5px solid ${acc.color}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>{acc.avatar}</div>
        ...
        {showCreate && isAdmin && (
          <div style={{ padding: "20px 24px", borderBottom: "1px solid #F1F3F5", background: "#FAFAFA" }}>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {AVATAR_LIST.map(emoji => (
                  <button key={emoji} onClick={() => setUserForm(f => ({ ...f, avatar: emoji }))}
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {COLOR_LIST.map(color => (
                  <button key={color} onClick={() => setUserForm(f => ({ ...f, color }))}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: "#fff", border: "1px solid #E8EAED", borderRadius: 10, marginBottom: 14 }}>
              <div style={{ width: 38, height: 38, borderRadius: "50%", background: userForm.color + "22", border: `2px solid ${userForm.color}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>{userForm.avatar}</div>
              <div>
            ...
            {formError && (
              <div style={{ background: "#FFF5F5", border: "1px solid #FFE3E3", borderRadius: 8, padding: "8px 12px", marginBottom: 12, fontSize: 12, color: "#E03131" }}>⚠️ {formError}</div>
            )}
          ...
          <div style={{ padding: "20px 24px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, paddingBottom: 16, borderBottom: "1px solid #F1F3F5" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 44, height: 44, borderRadius: "50%", background: member.color + "22", border: `2px solid ${member.color}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }}>{member.avatar}</div>
                <div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {/* Supprimer (Admin seulement, pas sur soi-même) */}
                ...
                <Btn onClick={handleSaveRights} variant="blue" size="sm">{saved ? "✅ Sauvegardé !" : "💾 Sauvegarder"}</Btn>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                {Object.values(SCREENS).map(screen => {
                  ...
                  return (
                    <div key={screen.id} onClick={() => !isAdminSel && toggleScreen(screen.id)}
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {Object.values(ACTIONS).map(action => {
                  ...
                  return (
                    <div key={action.id} onClick={() => !isAdminSel && toggleAction(action.id)}
